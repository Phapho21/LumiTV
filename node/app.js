			...
			var loadContent = function (obj) {
				for(var i=0, l=obj.list.length; i<l; i++) {
					if(obj.list[i].hasOwnProperty("id") && obj.list[i].id!="") {
						...
						obj.list[i].searchTag = parseTAGfromHTML2(getFileData("../component/" + obj.list[i].id + ".html"));
					}
					if(obj.list[i].hasOwnProperty("list") && obj.list[i].list.length) {
						loadContent(obj.list[i]);
					}
					//console.log(obj.list[i].content);
				}
			}
			//console.log(menuJson);
...
function parseTAGfromHTML (data) {
	...
	var pattern1 = new RegExp("searchWord\\s*?=\\s*?['\"]?\\s*(\.*[^ ]*?)['\"]", 'ig');
...
function parseTAGfromHTML2 (data) {
	var retData = [];

	var pattern2 = new RegExp("linkTag\\s*?=\\s*?['\"]?\\s*(\.*[^ ]*?)['\"]", 'ig');
	while(res2 = pattern2.exec(data)) {
		console.log(res2[1])
		retData.push({"tag2": res2[1], "index": res2.index});
	}
	return retData;
}
