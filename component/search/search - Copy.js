...
var Search = (function () {
	...
	Component.render = function () {
		...
		$("<div>").addClass("heading").html($("<div>").addClass("title").html("Result for <span style='color: #000;'>'" + searchTextToShow + "'</span>")).appendTo(pageContent);
		...
		$("section#introContent").append(pageContent);
	}
	...
	Component.highlightText = function (data) {
		...
		searchText = searchText.replace(searchRegExp, "<b>$1</b>");
		if (result && result.length>1) {
			for (var i=0, l=result.length; i<l; i++) {
		...
		if (this.searchResult.tag.length) {
			for (var i=0, l=this.searchResult.tag.length; i<l; i++) {
				...
				var container = $("<div>").addClass("search_container").click((function (obj) {
					return function () {
						...
						if (obj.hasOwnProperty("id") && obj.id!="") {
							window.parent.Menu.searchMode = 'searchFocus';
							window.parent.Menu.searchData = obj.content;
							$(window.parent.document.querySelector("iframe#contentFrame")).data('mode', 'searchFocus');
							window.parent.document.querySelector("iframe#contentFrame").src = 'component/' + obj.id + '.html';
						}
					}
				})(resultRow));
				...
				if (id) {
					...
					}).css({'pointer-events': 'none'}).data('tag', {
						'title': title
